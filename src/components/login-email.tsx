...
export function LoginEmail() {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    if (!normalizedEmail || !normalizedEmail.includes("@")) return setError("Enter a valid email address.");
    if (password.length < 8) return setError("Use a password with at least 8 characters.");
    setLoading(true);
    const supabase = createClient();
    const { data, error: authError } = await supabase.auth.signInWithPassword({ email: normalizedEmail, password });
    setLoading(false);
    if (authError) return setError(authError.message.toLowerCase().includes("invalid login credentials") ? "Incorrect email or password." : authError.message);
    if (!data.user) return setError("We could not start your session. Please try again.");
