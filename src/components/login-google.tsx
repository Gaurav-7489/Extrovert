...
export function LoginGoogle({ label = "Continue with Google" }: { label?: string }) {
  ...
  return (
    <div>
      <button type="button" onClick={signIn} disabled={loading} className="flex h-13 w-full items-center justify-between rounded-2xl bg-zinc-950 px-5 text-sm font-black text-white shadow-lg shadow-zinc-950/10 disabled:cursor-wait disabled:opacity-70">
        <span className="flex items-center gap-3"><span aria-hidden="true" className="grid h-6 w-6 place-items-center rounded-full bg-white text-sm font-black text-zinc-950">G</span><span>{loading ? "Connecting to Google…" : label}</span></span>
