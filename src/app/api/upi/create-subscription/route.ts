...
export async function POST(request: Request) {
  try {
    ...
    const { error } = await admin.from("upi_payment_submissions").insert({
      id: paymentId,
      user_id: user.id,
      payment_type: "subscription",
      product: plan,
      amount_paise: config.amountPaise,
      metadata: { plan, upi_id: "gauravbhardwaj7489@okaxis" },
    });
    if (error) throw error;

    return NextResponse.json({ success: true, paymentId, plan, amount: config.amountPaise, currency: "INR", upiId: "gauravbhardwaj7489@okaxis", upiUrl: createUpiPaymentUrl({ amountPaise: config.amountPaise, note: config.label }) });
  } catch (error) {
