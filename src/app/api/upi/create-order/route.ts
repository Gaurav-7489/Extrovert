...
export async function POST(request: Request) {
  try {
    ...
    const { error: submissionError } = await admin.from("upi_payment_submissions").insert({
      id: upiPaymentId,
      user_id: user.id,
      payment_type: "shop",
      product,
      amount_paise: config.amountPaise,
      metadata: { shop_order_id: shopOrderId, upi_id: "gauravbhardwaj7489@okaxis" },
    });
    ...
    return NextResponse.json({
      ...
      currency: "INR",
      upiId: "gauravbhardwaj7489@okaxis",
      upiUrl: createUpiPaymentUrl({ amountPaise: config.amountPaise, note: `Extrovert ${config.label}` }),
    });
  } catch (error) {
