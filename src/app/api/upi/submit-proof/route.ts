...
export async function POST(request: Request) {
  try {
    ...
    const { error: updateError } = await admin.from("upi_payment_submissions").update({ utr, status: "pending", updated_at: new Date().toISOString() }).eq("id", paymentId).eq("user_id", user.id);
