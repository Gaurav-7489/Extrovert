...
import { ArrowRight, Heart, MessageCircle, ShieldCheck, Users, type LucideIcon } from "lucide-react";
import { routes } from "@/config/routes";
import BrandLogo from "@/components/BrandLogo";

const features: { icon: LucideIcon; title: string; text: string }[] = [
  { icon: Users, title: "Discover", text: "Find people that fit your preferences." },
  { icon: Heart, title: "Match", text: "Like each other and connect." },
  { icon: MessageCircle, title: "Chat", text: "Move from a match to a conversation." },
];
  ...
  return (
    <main className="min-h-screen bg-[#0a0a0c] font-sans text-zinc-100">
      ...
      <section className="relative overflow-hidden px-5 pb-20 pt-20 sm:px-6 sm:pt-28">
        ...
        <div className="relative mx-auto max-w-4xl text-center">
          ...
          <div className="mt-7 flex justify-center gap-3">
            ...
            <Link href={routes.safety} className="inline-flex items-center gap-2 rounded-full border border-white/10 bg-[#121216] px-6 py-3.5 text-sm font-bold text-zinc-200 hover:bg-white/5">Safety</Link>
          </div>
        </div>
      </section>
      ...
      <section className="border-y border-white/10 bg-[#121216] px-5 py-10 sm:px-6">
        <div className="mx-auto grid max-w-4xl gap-3 sm:grid-cols-3">
          {features.map(({ icon: Icon, title, text }) => (
            <article key={title} className="rounded-3xl border border-white/10 bg-[#16161d] p-5">
              <div className="grid h-10 w-10 place-items-center rounded-2xl bg-[#550000]/20 text-red-300"><Icon className="h-5 w-5" /></div>
      ...
      <section className="px-5 py-12 sm:px-6">
        <div className="mx-auto flex max-w-4xl items-center gap-3 rounded-3xl border border-white/10 bg-[#121216] p-5"><ShieldCheck className="h-5 w-5 shrink-0 text-red-300" /><p className="text-xs leading-5 text-zinc-400">Safety, verification, blocking, reporting, and privacy controls are built into Extrovert.</p></div>
      </section>

      <footer className="border-t border-white/10 px-5 py-7 sm:px-6">
        <div className="mx-auto flex max-w-4xl items-center justify-between text-[11px] text-zinc-500"><span>© {new Date().getFullYear()} Extrovert</span><div className="flex gap-4"><Link href={routes.privacy}>Privacy</Link><Link href={routes.terms}>Terms</Link><Link href={routes.safety}>Safety</Link></div></div>
      </footer>
    </main>
  );
}
