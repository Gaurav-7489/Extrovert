...
export default async function DiscoverPage(){
  ...
  const [{data:myProfile},{data:myPrefs},{data:isPro}]=await Promise.all([
    supabase.from("profiles").select("id,profile_completed").eq("id",userId).maybeSingle(),
  ...
  const {data:rawProfiles,error}=await supabase.rpc("get_discover_profiles_v2",{
    p_excluded_ids:Array.from(excludedIds),
  ...
  const profilesWithPhotoUrls=ranked.map(profile=>{
    ...
    return{...profile,profile_photo_url:photos[0]?.url??sharedPhoto,profile_photos:photos,verification_status:profile.verification_status,area_verification_status:profile.area_verification_status,area_name:profile.area_name,bio:profile.bio??null,identity_context:context,identity_verified:identityVerified,area_verified:areaVerified};
  });
