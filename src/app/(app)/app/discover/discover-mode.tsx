...
export default function DiscoverMode({ profiles, isPro = false, nearbyArea = null }: Props) {
  ...
  return (
    <div className="extrovert-discover mx-auto flex w-full max-w-[33rem] flex-col px-3 pt-3 font-sans sm:px-4">
      <div className="mb-3 grid grid-cols-2 rounded-2xl border border-zinc-200/90 bg-zinc-100/90 p-1 shadow-sm transition-colors dark:border-white/10 dark:bg-[#141419]" role="tablist" aria-label="Discover mode">
      ...
      {mode === "nearby" && nearbyProfiles.length === 0 ? (
        <main className="flex min-h-[calc(100dvh-200px)] items-center justify-center px-2 py-6">
          <section className="w-full rounded-[2rem] border border-zinc-200/90 bg-white p-7 text-center shadow-lg transition-colors dark:border-white/10 dark:bg-[#121216] dark:shadow-2xl sm:p-8">
            ...
            <h1 className="mt-1 text-2xl font-bold tracking-tight text-zinc-950 dark:text-zinc-50">{nearbyArea ? "No one nearby yet" : "Set your area to discover nearby"}</h1>
            <p className="mx-auto mt-2 max-w-xs text-xs leading-relaxed text-zinc-500 dark:text-zinc-400 sm:text-sm">{nearbyArea ? "Try switching to For You to discover people in neighboring circles. Nearby shows members in your verified zone without exposing exact locations." : "Add and verify your area in profile setup to see members in your local circle."}</p>
