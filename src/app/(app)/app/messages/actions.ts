...
export async function loadOlderMessages(matchId:string,before:string):Promise<OlderMessagesResult>{
  ...
  const {data,error}=await supabase.from("messages").select("id,sender_id,content,ciphertext,encryption_version,created_at").eq("match_id",matchId).lt("created_at",new Date(cursor).toISOString()).order("created_at",{ascending:false}).limit(41);
