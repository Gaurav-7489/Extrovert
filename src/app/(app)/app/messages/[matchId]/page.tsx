...
export const metadata: Metadata = { title: "Secure Chat | Extrovert" };
...
export default async function ChatPage({ params }: Props) {
  ...
  const [{ data: blockRecord }, { data: profileRows, error: profileError }, { data: identity }, { data: messages, error: messagesError }] = await Promise.all([
  ...
  const verifiedProfile = { ...profile, verification_status: identity?.verification_status ?? profile.verification_status ?? null };

  return <div className="mx-auto flex h-full min-h-0 w-full max-w-2xl flex-col overflow-hidden font-sans md:border-x md:border-zinc-100">
    <ChatLoader matchId={matchId} currentUserId={userId} otherUserId={otherUserId} otherProfile={verifiedProfile} otherPhotoUrl={photoUrl} initialMessages={[...(messages ?? [])].reverse() as StoredMessage[]} />
  </div>;
}
